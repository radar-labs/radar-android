...
object Bech32 {
  ...
  fun convert(data: ByteArray, inBits: Int, outBits: Int, pad: Boolean): ByteArray {
    ...
    for (dataValue in data) {
      value = (value shl inBits) or dataValue.toInt()
      bits += inBits
    ...
    if (pad) {
      if (bits > 0) result.add((value shl (outBits - bits)) and maxV)
    } else {
      if (bits >= inBits) throw AddressFormatException.InvalidDataLength("Excess padding")

      if ((value shl (outBits - bits)) and maxV > 0) {
        throw AddressFormatException.InvalidDataLength("Non-zero padding")
      }
    }
