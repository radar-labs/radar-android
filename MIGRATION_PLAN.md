...
- [ ] `aba8376294` | 04-30 | feature | Add the new edit username screen (#2) → new Radar payment-username screen + wire into payment settings/send flow. (skip: deleted iOS test jpg)
...
- [ ] `b35e77bc82` | 05-01 | feature | Hide balance/amount from receive banner and low balance alert (#9) → hide amounts in banner/alert (distinct from existing eye-toggle).
- [ ] `d0ae4ae661` | 05-01 | feature | New payment settings (#8) → new payment settings menu + AppSettings wiring + string. (skip: iOS CLAUDE.md, ci_scripts/test_apns_push.py)
- [ ] `7ac08be7bf` | 05-01 | bugfix | Fix for crash on filtering chat list by gesture (#10) → swipe-filter crash.
- [ ] `f1a85d3fbe` | 05-07 | feature | finish the new send payment contact selection flow (#11) → send-payment recipient selection.
- [ ] `b0bd09c807` | 05-07 | branding | App name→Radar / icon / "sat"→"sats" (#12) → launcher icon + sat→sats. (dedup: app_name already Radar)
- [ ] `07343eab15` | 05-08 | feature | Add the payment onboarding flow screens → onboarding (intro, add-funds intro, deposit-received, username onboarding, setup-complete, splash) + coordinator.
- [ ] `bc4dd06d2b` | 05-10 | bugfix | Fix payments onboarding flow and add funds screen (#13) → onboarding fixes + QR generator (Cake-compatible).
- [ ] `c90dd2d790` | 05-10 | feature | Add chat interface payment UI updates (#14) → chat payment bubbles + send-confirm screen.
