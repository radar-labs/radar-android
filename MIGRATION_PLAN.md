...
- [x] `85485a8b7d` | 05-18 | bugfix | Fix compatability with cake (#23) → **DONE.** `BreezSdkWrapper.connect` now seeds the SDK from `Seed.Mnemonic(bip39EntropyToMnemonic(entropy))` instead of `Seed.Entropy` so the backup phrase is BIP-39-interoperable (Cake / other wallets). Mirrors iOS. See DECISIONS.md.
- [d] `1ce180c53e` | 05-18 | branding | Change app icon (#24) → **DEFERRED (asset).** Second Radar icon revision. Bundled with the launcher-icon asset task (`b0bd09c807` #12) — apply once with the Radar logo as Android assets. See ISSUES.md.
- [s] `18c479ada0` | 05-18 | bugfix | Fix send payment crashing + minor UI → **SKIPPED — iOS-platform-specific.** Crash was `UIView.matchWidthsOfViews` called before adding views to a UIStackView (UIKit constraint ordering) + a note-field paste icon. Android's send flow is `CreatePaymentFragment` (different rendering); no `matchWidthsOfViews` analog. See DECISIONS.md.
- [x] `5b4b9f52f0` | 05-18 | branding | Replace Signal/MobileCoin refs with Radar/Lightning → **DONE (payment-scoped).** Rebranded "Signal"→"Radar" in 9 payment-context strings (PaymentsHome/PaymentsTransfer/DeactivateWallet) that still named the app "Signal". MobileCoin residual was already cleared (#4). Broader non-payment "Signal" refs left as-is (protocol/feature/legal — iOS's EN change was payment-scoped too). See DECISIONS.md.
