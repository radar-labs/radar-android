...
## 2026-05-28 — Breez SDK 0.9.1 → 0.14.0: `newAddress=null` for on-chain receive
**Context:** Bump from 0.9.1 to 0.14.0 made `ReceivePaymentMethod.BitcoinAddress` a data class with a `newAddress: Boolean?` field (was a no-arg singleton). The semantics: `true` = force a fresh address on every call, `false` = reuse the same address, `null` = let the SDK decide.
**Decision:** Pass `null`. This preserves the implicit 0.9.1 behavior (no parameter = SDK default) and avoids two failure modes: (a) `true` would generate a brand-new address each `getOnchainAddress()` call, breaking address reuse and confusing peers who saved an older address; (b) `false` would force reuse even when the SDK might prefer a fresh one for privacy/internal reasons. `null` defers the choice to the SDK and matches the iOS call site shape (iOS also doesn't override `newAddress`).
**Confidence:** medium — not UI-run; assumes the SDK's default is sensible. **Revisit if:** the user reports the on-chain address changing on every refresh (then explicitly set `false`) or that addresses are being reused too aggressively for privacy (then `true`).
**Other API delta (no decision needed):** `PrepareLnurlPayRequest`'s `amountSats: ULong` → `amount: BigInteger` (direct rename + type widening); `optionalValidateSuccessActionUrl` positional → `validateSuccessActionUrl: Boolean?` named.
